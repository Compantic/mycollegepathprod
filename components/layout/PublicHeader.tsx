...
import { Sparkles, Zap, CreditCard, Medal, Menu } from "lucide-react";
...
const navItems = [
  ...
  { href: "/honoring", label: "Honoring Those Who Serve", icon: Medal },
];
    ...
    >
      ...
      >
        <div className="mx-auto flex flex-wrap items-center justify-between gap-2 sm:gap-4 px-4 py-2 sm:h-16 max-w-6xl sm:px-6 lg:px-8">
          ...
          >
            <LogoIcon className="h-7 w-7 text-primary-600" />
          ...
          <nav className="hidden sm:flex items-center gap-1" aria-label="Main navigation">
            {navItems.map(({ href, label, icon: Icon }) => {
              const isActive =
                ...
                (href === "/honoring" && pathname === "/honoring");
              return (
                <Link
                  key={href}
                  href={href}
                ...
                >
                  <Icon className={cn("h-4 w-4", isActive ? "text-white" : "text-primary-500/80")} aria-hidden />
                  {label}
                </Link>
              );
            })}
